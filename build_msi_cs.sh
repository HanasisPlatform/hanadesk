...
print_usage() {
  echo "Usage: ./build_msi_cs.sh <version> [--skip-cargo] [--version-up] [--sign]"
  exit 1
}
...
BUILD_COMMAND="python build.py --flutter --cs --skip-portable-pack "
...
echo "Changing directory to res/msi_cs..."
cd res/msi_cs || { echo "Directory /res/msi_cs does not exist. Exiting..."; exit 1; }
...
if [ "$VERSION_UP" == "--version-up" ]; then
  ...
  python preprocess.py --arp -d ../../HanaDeskCS --version "$VERSION" --app-name "HanaDesk"
else 
...
MSI_FILE="../../HanaDeskCS-${VERSION}.msi"
