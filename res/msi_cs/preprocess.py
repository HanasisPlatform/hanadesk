...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="HanaDesk", help="The app name."
    )
    parser.add_argument(
...
def gen_pre_vars(args, dist_dir):
    ...
    print("end gen pre vars")
    print(
        f'data = {gen_content_between_tags(
...
def init_global_vars(dist_dir, app_name, args):
    ...
    if not version_pattern.match(g_version):
        raise ValueError(f"Error: version {g_version} not found in {dist_app}")
    ...
    try:
        ...
        # 빌드 날짜가 없거나 패턴에 맞지 않으면 현재 날짜/시간 사용
        if not g_build_date or not re.match(
            r"\d{4}-\d{2}-\d{2} \d{2}:\d{2}", g_build_date
        ):
            g_build_date = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    except Exception:
        g_build_date = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")

    print(f"Version: {g_version}, Build Date: {g_build_date}")
    print(f"Args: {args}")
...
def update_license_file(app_name):
    ...
    license_content = re.sub(
        "Purslane Ltd", app_name, license_content, flags=re.IGNORECASE
    )
    with open(license_file, "w", encoding="utf-8") as f:
        f.write(license_content)

    print("End updating license file...")


def replace_component_guids_in_wxs():

    print("Replacing component guids...")
    ...
    print("End replacing component guids...")
