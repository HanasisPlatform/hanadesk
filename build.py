...
def make_parser():
    ...
    parser.add_argument(
        "--cs", action="store_true", help="Build for CS package", default=False
    )
...
def build_flutter_deb(version, features):
    ...
    system2(f"cp -r {flutter_build_dir}/* tmpdeb/usr/lib/rustdesk/")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2("mkdir -p tmpdeb/usr/share/polkit-1/actions")
    system2("rm tmpdeb/usr/bin/rustdesk || true")
    system2(f"cp -r ../{binary_folder}/* tmpdeb/usr/lib/rustdesk/")
    system2("cp ../res/rustdesk.service tmpdeb/usr/share/rustdesk/files/systemd/")
    system2(
        "cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/rustdesk.png"
    )
...
def sign_with_safenet(exe_path):
    """SafeNet을 이용한 코드 서명 함수"""
    timestamp_url = "http://timestamp.digicert.com"
    cert_name = "HANASIS CO., LTD."

    try:
        print(f"Signing {exe_path} using SafeNet...")
        command = (
            f'signtool sign /n "{cert_name}" '
            f'/tr "{timestamp_url}" /td SHA256 /fd SHA256 "{exe_path}"'
        )
        system2(command)
...
def build_flutter_windows_for_cs(version, features, skip_portable_pack, args) -> None:
    if not skip_cargo:
        system2(f"cargo build --features {features} --lib --release")
    ...
    system2("flutter build windows --release --dart-define=FLAVOR=cs")

    print(f"{flutter_build_dir}HanaDesk_CS.exe")

    if os.path.exists(f"{flutter_build_dir}HanaDesk_CS.exe"):
        os.remove(f"{flutter_build_dir}HanaDesk_CS.exe")
        print("removed... existed HanaDesk_CS.exe")

    if os.path.exists(f"{flutter_build_dir}HanaDesk.exe"):
        os.remove(f"{flutter_build_dir}HanaDesk.exe")
        print("removed... existed HanaDesk.exe")
    ...
    copy_folder_contents(
        flutter_build_dir_2, f"{os.path.abspath(os.curdir)}/HanaDeskCS"
    )
    ...
    try:
        system2(
            f"python ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}HanaDesk_CS.exe"
        )

    except Exception as e:
        print(f"Error executing generate.py: {e}")
        raise
    ...
    if os.path.exists(f"./HanaDesk_CS-{version}-install.exe"):
        os.remove(f"./HanaDesk_CS-{version}-install.exe")
    os.rename("./rustdesk_portable.exe", f"./HanaDesk_CS-{version}-install.exe")
    print(
        f"output location: {os.path.abspath(os.curdir)}/HanaDesk_CS-{version}-install.exe"
    )

    os.makedirs(f"{os.path.abspath(os.curdir)}/HanaDeskCS", exist_ok=True)

    shutil.copy2(
        f"./HanaDesk_CS-{version}-install.exe",
        f"{os.path.abspath(os.curdir)}/HanaDeskCS",
    )

    if os.path.exists(f"{os.path.abspath(os.curdir)}/HanaDeskCS/HanaDesk.exe"):
        os.remove(f"{os.path.abspath(os.curdir)}/HanaDeskCS/HanaDesk.exe")

    os.rename(
        f"{os.path.abspath(os.curdir)}/HanaDeskCS/HanaDesk_CS-{version}-install.exe",
        f"{os.path.abspath(os.curdir)}/HanaDeskCS/HanaDesk.exe",
    )

    if args.sign:
        exe_path = f"{os.path.abspath(os.curdir)}/HanaDeskCS/HanaDesk.exe"
        sign_with_safenet(exe_path)

    print(f"output location: {os.path.abspath(os.curdir)}/HanaDeskCS/HanaDesk.exe")
...
def build_flutter_windows(version, features, skip_portable_pack, args):
    ...
    system2("flutter build windows --release --dart-define=FLAVOR=member")

    print(f"{flutter_build_dir}HanaDesk.exe")
    ...
    copy_folder_contents(flutter_build_dir_2, f"{os.path.abspath(os.curdir)}/HanaDesk")
    ...
    try:
        ...
        system2(
            f"python ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}HanaDesk.exe"
        )
    except Exception as e:
        print(f"Error executing generate.py: {e}")
        raise
    ...
    if os.path.exists(f"./HanaDesk-{version}-install.exe"):
        os.remove(f"./HanaDesk-{version}-install.exe")
    os.rename("./rustdesk_portable.exe", f"./HanaDesk-{version}-install.exe")
    print(
        f"output location: {os.path.abspath(os.curdir)}/HanaDesk-{version}-install.exe"
    )

    os.makedirs(f"{os.path.abspath(os.curdir)}/HanaDesk", exist_ok=True)
    shutil.copy2(
        f"./HanaDesk-{version}-install.exe", f"{os.path.abspath(os.curdir)}/HanaDesk"
    )

    if os.path.exists(f"{os.path.abspath(os.curdir)}/HanaDesk/HanaDesk.exe"):
        os.remove(f"{os.path.abspath(os.curdir)}/HanaDesk/HanaDesk.exe")

    os.rename(
        f"{os.path.abspath(os.curdir)}/HanaDesk/HanaDesk-{version}-install.exe",
        f"{os.path.abspath(os.curdir)}/HanaDesk/HanaDesk.exe",
    )

    if args.sign:
        exe_path = f"{os.path.abspath(os.curdir)}/HanaDesk/HanaDesk.exe"
        sign_with_safenet(exe_path)

    print(f"output location: {os.path.abspath(os.curdir)}/HanaDesk/HanaDesk.exe")
...
def main():
    ...
    if windows:
        ...
        if flutter:
            ...
            if cs:
                build_flutter_windows_for_cs(
                    version, features, args.skip_portable_pack, args
                )
                return
            else:
